...
while True:
    ...
    subprocess.run(["python", "./train_net.py", "--path", "./networks/target_classifier.pt", "--dataset", "0"], stdout=FNULL, stderr=FNULL)
    for i in range(1, 11):
        subprocess.run(["python", "./train_net.py", "--path", "./networks/classifiers/classifier_{}.pt".format(i), "--dataset", "{}".format(i)], stdout=FNULL, stderr=FNULL)
